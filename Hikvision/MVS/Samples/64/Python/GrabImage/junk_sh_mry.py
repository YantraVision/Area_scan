...
def work_thread(cam=0, pData=0, nDataSize=0):
        ...
        while True:
                ...
                if ret == 0:
                    ...
                    arr=resized_arr 
                    #shm_b = shared_memory.SharedMemory(create=True, size=arr.nbytes)
        
        #b_arr = np.ndarray(arr.shape, dtype=arr.dtype, buffer=shm_a.buf)
                    b_arr[:] = arr[:]  # Copy the original data into shared memory
                    #print(b_arr,"start")
                    #existing_shm = shared_memory.SharedMemory(name=shm_a.name)
                    #c_arr = np.ndarray(b_arr.shape, dtype=b_arr.dtype, buffer=existing_shm.buf)
...
class VideoThread(QThread):
    ...
    def run(self):
        while True:
            ...
            print(b_arr,"video")
            time.sleep(1)
            self.change_pixmap_signal.emit(b_arr)
